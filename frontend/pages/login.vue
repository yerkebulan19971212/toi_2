  ...
  <div class="min-h-screen bg-cream-100 flex items-center justify-center px-4">
    <div class="w-full max-w-md">
      <div class="text-center mb-8">
        ...
        <h1 class="font-serif text-2xl font-bold text-gray-900">Кіру</h1>
        <p class="text-gray-500 font-sans text-sm mt-1">Аккаунтыңызға кіріңіз</p>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-sm p-8">
        ...
        <p class="text-center text-sm font-sans text-gray-500 mt-6">
          Аккаунтыңыз жоқ па?
          <NuxtLink to="/register" class="text-brand-green font-medium hover:underline">Тіркелу</NuxtLink>
        </p>
      </div>
    </div>
  </div>
...
if (authStore.isLoggedIn) {
  await navigateTo(route.query.redirect || '/my-invitations')
}
...
const submit = async () => {
  ...
  try {
    ...
    error.value = 'Логин немесе құпия сөз қате'
  } finally {
    loading.value = false
  }
}
